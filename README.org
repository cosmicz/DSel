...
* Overview
:PROPERTIES:
:ID:       c40fd4da-bb5c-4661-9c38-6c353a6f4b2e
:END:
...
** Module (=dsel-module=) Async API (=dsel-defmodule= and =dsel-defaforward=)

=dsel-module= is the base class for all runnable components in DSel.
DSel provides powerful macros for creating modular, composable LLM applications with async-first design:

*** Creating Custom Modules

Use =dsel-defmodule= to create custom modules that combine multiple predictors:
...
Use =dsel-defaforward= to define async forward methods for your modules:
...
Use =dsel-aio-then= for non-blocking async execution with callbacks:
...
Use =dsel-forward= for blocking synchronous execution when needed:
...
(dsel-defaforward rag-module (&key query)
  ...
  (let* ((retriever (cdr (assq 'retriever submodules)))
         ...
         (retrieval-results (dsel-aio-await
