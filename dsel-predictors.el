...
(cl-defstruct (dsel-predict (:include dsel-module))
  ...
  demos                                 ; list of dsel-example: few-shot examples
  ...
  (let* ((lm-to-use (or (dsel-predict-lm predict) dsel-settings--lm))
         ...
         (prediction
          (apply #'dsel-make-prediction
                 (append
                  ;; Add all input and output fields
                  (cl-loop for (field . value) in current-inputs-alist
                         collect field collect value)
                  (cl-loop for (field . value) in parsed-outputs-alist
                         collect field collect value)
